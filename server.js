...
if (!apiKey) {
  ...
  console.error(
    "  2. O arquivo se chama exatamente '.env' (e não '.env.txt' ou 'env')."
  );
  console.error(
    "  3. Dentro do .env, a linha é: GEMINI_API_KEY=SUA_CHAVE_AQUI"
  );
...
app.post("/chatAmina", async (req, res) => {
  ...
  try {
    ...
    const chat = model.startChat({
      history: [
        ...
        },
        {
          ...
          parts: [
            {
              text: "Entendido. Serei a Amina, uma amiga virtual acolhedora e empática, pronta para ouvir e apoiar. Se for uma emergência, recomendarei o 190 ou 180.",
            },
          ],
        },
      ],
    ...
    res
      .status(500)
      .json({ error: "Não consegui pensar em uma resposta agora." });
  }
});
